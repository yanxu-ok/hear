<template>
	<view>
		<view v-if="hasOverlay" ref="myp-popo-overlay" :class="['myp-popo-over', 'myp-bg-'+overlay.bgType]" @tap.stop="overlayClose"
		 :style="mrOverlayStyle + overlayNoWeexAni">
			<slot name="overlay"></slot>
		</view>
		<view ref="myp-popo" bubble="true" @touchstart="onAllTouchStart" @touchmove="onAllTouchMove" @touchcancel="onAllTouchCancel"
		 @touchend="onAllTouchEnd" @tap.stop="toPrevent" :class="['myp-flex-column', 'myp-popo', 'myp-bg-'+bgType]" :style="boxStyle+mrPopStyle + noWeexAni">
			<view v-if="!allowAll" ref="myp-popo-stand" @touchstart="onStandTouchStart" @touchmove="onStandTouchMove"
			 @touchcancel="onStandTouchCancel" @touchend="onStandTouchEnd" @tap.stop="toPrevent">
	...
	export default {
		...
		computed: {
			...
			widthPx() {
				...
				if (w <= 0) {
					return this.screenWidth - this.leftPx - this.rightPx - (this.leftOffsetPx >= 0 ? this.leftOffsetPx : 0) - (this.rightOffsetPx >=
						0 ? this.rightOffsetPx : 0)
				}
				return w
			},
			...
			heightPx() {
				...
				if (h <= 0) {
					return this.screenHeight - this.topPx - this.bottomPx - (this.topOffsetPx >= 0 ? this.topOffsetPx : 0) - (this.bottomOffsetPx >=
						0 ? this.bottomOffsetPx : 0) - this.extraPx
				}
		...
		methods: {
			...
			overlayClose(e) {
				...
				// 先判断状态是否打开 然后才暴露方法
				this.isShow && this.$emit('handleClose')
			},
				...
				if (!this.isShow) {
					if (this.pos === 'top') {
						if (offsetY > 0) {
							...
							if (this.hasOverlay) {
								this.overlayNoWeexAni =
									`transition-property:opacity; transition-duration: 0ms;opacity:${y/maxSize};height:${this.overlayHeight}px;`
							}
						} else {}
						...
						if (offsetX > 0) {
							...
							if (this.hasOverlay) {
								this.overlayNoWeexAni =
									`transition-property:opacity; transition-duration: 0ms;opacity:${x/maxSize};height:${this.overlayHeight}px;`
							}
						} else {}
			...
			noWeexAppearPopup(bool, duration = this.duration) {
				...
				// console.log(this.noWeexAni,'滚动的样式');
				// 下面是遮罩层overlay的样式
				const that = this
				setTimeout(() => {
					!bool && (that.noWeexAni = '')
			...
			// 获取需要移动的size 自身的大小减去露出去的高度就是需要变换的
