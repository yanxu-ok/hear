...
<script>
	...
	export default {
		...
		methods: {
			...mapActions(['get_comment_list', 'get_comment_manager_token', 'praise', 'cancel_praise']),
			...
			async getCommentList(page) {
				...
				let result = await this.get_comment_list(data)
				return result
			},
				...
				if (item.isPraise == 0) {
					...
					if (result.ok) {
						...
						if (!oIndex) {
							...
							this.dataList[oIndex].children[index].praiseNum++
						}
					}
