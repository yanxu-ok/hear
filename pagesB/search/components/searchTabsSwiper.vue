<template>
	<view class="search_swiper_contain">
		...
		<template v-if="show">
			<u-empty mode="search" style="margin-top: 20rpx;"></u-empty>
		</template>
	</view>
</template>
...
<script>
	...
	export default {
		...
		async mounted() {
			this.categoryList = await this.get_search_category(this.keyword)
			console.log(this.categoryList, '有没有东西');
		...
		methods: {
			...
			...mapActions(['get_topic_by_categoryid', 'get_all_category', 'get_search_category']),
